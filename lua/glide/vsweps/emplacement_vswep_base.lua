...
if SERVER then
    ...
    VSWEP.ProjectileDamageMult = 1
    -- QOL thing for VSWEPS that fire things like missiles and RPG's ( TOW and SPG for example ), multiplies the dmg by the number put
    ...
    VSWEP.EnableReloadSequence = false 
    -- Mainly for models that have disposable elements as bodygroups ( tubes, ammo boxes, etc ), such as the TOW
    VSWEP.ReloadSequenceBodyGroups = nil
    -- Same shit, has to be a table

    VSWEP.FiringAnimation = ""
    -- Self explanitory, you can set your own firing animation 
...
if CLIENT then
    VSWEP.LocalCrosshairOrigin = Glide.GetCameraAimPos()
...
if SERVER then
    ...
    function VSWEP:Reload()
        ...
        if self.ReloadEndSound ~= "" then
            self.Vehicle:EmitSound( self.ReloadEndSound )
        end
        if self.EnableReloadSequence and self.ReloadSequenceBodyGroups  then
            self.Vehicle:SetBodygroup( self.ReloadSequenceBodyGroups[3], self.ReloadSequenceBodyGroups[4] )  
        end
    end

    function VSWEP:ReloadSequence( reloadTime )
        timer.Simple( reloadTime / 2, function ()
            if self.Vehicle:IsValid() then
                self.Vehicle:EmitSound( self.ReloadStartSound )
                if self.ReloadSequenceBodyGroups then
                    self.Vehicle:SetBodygroup( self.ReloadSequenceBodyGroups[1], self.ReloadSequenceBodyGroups[2] )                                    
                end
            end
        end )
    end
    ...
    function VSWEP:PrimaryAttackInternal()
        ...
        if self.ammo < 1 and self.MaxAmmo > 0 then
            ...
            if self.EnableReloadSequence then
                self:ReloadSequence( self.ReloadDelay )
            end
        end
    end
...
if CLIENT then
    ...
    function VSWEP:DrawCrosshair()
        ...
        if IsValid( lockOnTarget ) then
            ...
            origin = vehicle:LocalToWorld( Glide.GetCameraAimPos() )
