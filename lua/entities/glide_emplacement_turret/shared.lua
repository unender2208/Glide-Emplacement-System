...
function ENT:SetupDataTables()
    ...
    self:NetworkVar( "Int", "ShellCasingsOffsetY") -- Or X, idfk 
    self:NetworkVar( "Angle", "LastBodyAngle" )

    self:NetworkVar( "Int", "ViewpunchMultiplier" ) -- Self explanitory
    ...
    self:NetworkVar( "String", "FiringAnimation") -- Self explanitory
end
...
function ENT:UpdateTurret( parent, body, t )
    ...
    if isFiring and t > self.nextFire then
        ...
        self:FireBullet( pos, ang, user, self:GetGunBody():GetRight() )
    end
end
...
function ENT:FireBullet( pos, ang, attacker, shellDir )
    ...
    -- Shells
    eff = EffectData()
    eff:SetOrigin( pos - dir * self:GetShellCasingsOffsetY() )
    eff:SetEntity( self )
    eff:SetMagnitude( 1 )
    eff:SetRadius( 5 )
    eff:SetScale( 1 )
