...
if CLIENT then
    ...
    function ENT:OnUpdateAnimations()
        ...
        function self:GetFirstPersonOffset( seatIndex, localEyePos )
            ...
            localEyePos[2] = localEyePos[2] + math.cos( rad ) * 40
            localEyePos[1] = localEyePos[1] + math.sin( rad ) * -40
           -- Camera.fov = 20
...
if SERVER then
    ...
    function ENT:CreateFeatures()
        ...
        self:CreateWeapon( "guided_missile_launcher", {
            MaxAmmo = 1,
            FireDelay = 1.0,
            ReloadDelay = 8.0,
            ProjectileOffsets = {Vector( 0, 0, 37.5 )},
            EnableReloadSequence = true,
            ReloadStartSound = "glide/glide_emplacement_base/glide_bgm71_tow/reloadstart.wav",
            ReloadEndSound = "glide/glide_emplacement_base/glide_bgm71_tow/reloadend.wav",
            SingleShotSound = "glide/glide_emplacement_base/glide_bgm71_tow/tow_lauch.wav",
        ...
        turret:SetLocalPos( Vector( -0, 4.3, 7 ) )
        turret:SetLocalAngles( Angle() )
        turret:Spawn()
        ...
        for _, w in ipairs( self.supports ) do
            local weld = constraint.Weld( self, w, 0, 0, 0, false, true ) -- Connects the created support to the gun/chassis itself
    ...
    function ENT:OnPostThink() -- Due to weird jittering effects on moving the seat in client, im doing it here. Sorry!
        ...
        local offset = Vector()
        local radius = 40 -- You dont HAVE to make a separate variable for the radius, just putting this here for anyone needing help reading
